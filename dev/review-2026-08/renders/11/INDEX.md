...
| `compare-difference-k3` | Three groups, so two contrasts under one indicator per pair. The three panels are unchanged and the supported panel is **unweighted** -- uniform width, plain ink, no sign colour -- because no pair has a single magnitude. The fixture is built rather than sliced: groups 1 and 2 share a chain, group 3 trades one link for another, and the three pairs the figure reports as supported (`v1-v5`, `v2-v3`, `v4-v5`) are exactly the three the construction moved. BEFORE refuses the fit outright, so its pane says so. |
| `compare-difference-k3-no-selection` | The same three groups with `difference_selection = FALSE`. There is no split and a pair has two magnitudes, so `plot()` draws the groups' own networks — the `type = "groups"` display, reached without asking for it. The three panels show the construction directly: groups 1 and 2 carry the shared chain, group 3 carries `v1-v5` instead of `v2-v3`. BEFORE refuses the fit. |
