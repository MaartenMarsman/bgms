...
| 10 edge-posterior panel redesign to the JASP/compendium standard (F-066; PIP wheel, prior overlay, conditional density; Savage-Dickey dots exactly when `edge_selection = FALSE`; extracts the `R/plot_style.R` house-style module) | Opus | report in 2026-08-01 — **awaiting MM's render judgment pre-merge** (the standing gate; PNGs in `wt-fix4/dev/review-2026-08/reports/assets/`). Lead verified: branch base `1508a888` IS the merge-base with develop (exactly what `4969f843` merged — no rebase; develop since base is dev/-record-only, so the branch's green gates transfer); the no-Jacobian frame claim source-verified end-to-end (GGM −0.5 conversion / mixed stores −Ω/2 / ordinal native); gates 1187/0/0 both tiers + 2 baseline NOTEs. Yields F-078 (mixed cross-edge naming, fixed), F-079 (compare fit refused — MM ratifies), F-066 implemented; future_tasks 42/43 filed. Lead accepted all 9 deviations (adaptive legend incl.). Open with MM: renders, 2-vs-3 decimals (lead rec 3 — flip in 11 if chosen), wheel fill (lead rec RB), F-079. Merge after MM approval; 12's `plot_bgms.R` contact resolved by merge order | `reports/10-edge-posterior-plot.md` (on branch, commits `a682b57e..8ca0f0f9`) |
...
| 13 remaining Phase-1 verifications + 09 follow-ups (0.1.6.3-vs-rc1 ordinal posterior agreement — the existing-user regression check, Phase-1 row 6; F-019 PD-guard settle; F-042 dead-fixture decision; F-073 guard closures — all four holes, incl. the planted-δ every-run pin design; F-074 `difference_scale` discriminator; F-075 follow-through per the apples-control verdict; F-077 optional; GGM fixture extending the report-10 slab-frame identity pin) | Opus | draft after 12 | — |
