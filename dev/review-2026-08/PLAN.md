...
| 19 **F-075 fix batch (release gate)**: `collapse_categories_across_groups()` recode semantics per MM's decision (union vs interim adjacent-merge; BC-exemption question); F-110 always-warn + store the map in the fit; F-111 Rd + comparison-vignette correction; NEWS behaviour-change entry with refit guidance; regression tests — the 8-line reproducer as a unit test + an estimate-survival assertion vs matched bgm() fits; POST-FIX re-measure of report 16's operating-characteristics table (same 10 seeds) incl. the healthy-edge slope (resolves the 09-3 side of F-074); F-113 real-data support-difference scan (ADHD fixture first); F-112 compare logp test hook IF MM pulls it forward | Opus | to draft — WAITING on MM's fix-shape sign-off (proposal in the lead's 2026-08-02 message) | — |
