...
| 14 NEWS.md verification + reconstruction for the 0.1.6.3 reader (F-001 blocker fix, F-023 sentence, F-029 line-by-line verification; claims table; NEWS.md-only branch) | Opus | done — **merged to develop** (commit `80a28dfb`). F-001 CLOSED (fixed pre-review by `45a4fae7` #187; agent re-verified line-by-line + added the pooled-everywhere clause); F-023/F-029 closed (7/11 F-029 items had NO entry); 98/98 pre-existing claims verified — 4 corrected (2 outright false at HEAD → F-082..F-085), 8 never-shipped dev entries removed; parse gate via the actual markdown-NEWS reader (33 rows clean). OUTSTANDING: MM's ratification read of the rewritten section + intent answers Q1 (D23 "released bgms" subject), Q3 (hierarchical block length — lead rec: keep for this release), Q4 (pairwise_scale family-preserving translation) | `reports/14-news-verification.md` |
