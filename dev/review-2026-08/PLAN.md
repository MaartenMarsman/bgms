...
| 12 nightly respec + release polish (F-071 T1/T2 split, workflows scheduled on develop; F-049 gate re-found + Rd bound; F-072 group labels; F-057 Rd numbering line; F-004 README line; D1-conditional PR fast gate) | Opus | **issued 2026-08-01**; status 2026-08-02: gates green — T1 CI 43.1 min success (run 30735988783, 0 FAIL/0 WARN), `--as-cran` 2 baseline NOTEs, CRAN-mode suite clean, classification covers all 98 slow blocks (38 T1 + 60 T2); full local suite (both env vars) still running; zratio-surface-build flake written up as unexplained intermittent (see F-101), probe shipped as safe interim; report + lead verify-and-merge pending | last gate running |
| 13 remaining Phase-1 verifications + 09 follow-ups + CI-red diagnoses (0.1.6.3-vs-rc1 ordinal posterior agreement — the existing-user regression check, Phase-1 row 6; F-019 PD-guard settle; F-042 dead-fixture decision; F-080/F-081/F-101 the three 2-core CI flakes, diagnosed together (12's evidence inherits: `local::.` diagnosis retracted, pass without it on deps-only runner, 4-worker-nodes-for-`cores = 2L` anomaly — F-101 row + report 12); F-100 compliance-harness property-name sweep + fixture re-baseline; F-073 guard closures — all four holes, incl. the planted-δ every-run pin design; F-074 discriminator — REUSE brief 16 Phase B(i)'s `difference_scale` sweep rather than rerunning; only the level-collapse variant on matched real-data-style fits if 16 leaves F-074 unsettled; F-077 optional; GGM fixture extending the report-10 slab-frame identity pin. F-075 program MOVED OUT to brief 16 per MM's release-gate ruling) | Opus | draft after 12 | — |
