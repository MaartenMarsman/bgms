...
| 21 mark prior-only compare summary rows (F-117) | Opus | **issued 2026-08-03, LIVE** — `briefs/21-summary-zero-marking.md` stands as written (MM re-ratified after the F-120 detour); MM ruled 2026-08-03: WIDEN + wording approved + format approved; F-121 = BLOCKER, diagnosis in the same relay; merge after the follow-up verifies | handed to MM 2026-08-03 |
| 22 compare slab Cauchy → Normal (F-119) + ridge visualization + zero-support difference-test evaluation | Opus | UNBLOCKED (13 merged) — brief drafting now | — |
| 23 mixed relational certification (vs OMRF sharp, vs GGM calibrated, cross-block planted recovery) | Opus | **issued 2026-08-03** — `briefs/23-mixed-relational-cert.md`; REPORT-ONLY; third concurrent, ~4 threads, one fit at a time | handed to MM 2026-08-03 |
| 24 precision_graph_prior default → "hierarchical" (F-010 superseded) + F-022 gauge wiring | Opus | GATED on MM's F-103 alarm ruling (the flip makes the gauge default-visible; a ~25% healthy-fit alarm rate ships with it as-is); NEWS as feature wording; memo + docs-site re-bake flag | — |
