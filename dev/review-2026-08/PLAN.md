...
| 12 nightly respec + release polish (F-071 T1/T2 split, workflows scheduled on develop; F-049 gate re-found + Rd bound; F-072 group labels; F-057 Rd numbering line; F-004 README line; D1-conditional PR fast gate) | Opus | **done — merged `f24ad3c8` 2026-08-02**. All four gates passed (full local suite 0F/0W; T1 CI 43.1 min green, run 30735988783; `--as-cran` 2 baseline NOTEs; 98/98 blocks classified 38 T1 / 60 T2). Lead verified: probe monotonicity in source, F-049 Rd sentence vs `anchor_curve.R:207-211`, F-072 label carry + numeric extractor contract, workflow triggers/gates/timeouts, both red develop runs' signatures (→ F-101 root cause). Spawned F-103 (gauge Linux red, parked T2 against contract) + F-104 (socket intermittent, probe = interim); T2 budget unproven until first Sunday on main; equivocal T2 calls ratified (full-defaults + size-cap fits first back if budget rises). | report 12 |
| 13 remaining Phase-1 verifications + 09 follow-ups + CI-red diagnoses (0.1.6.3-vs-rc1 ordinal posterior agreement — the existing-user regression check, Phase-1 row 6; F-019 PD-guard settle; F-042 dead-fixture decision; F-080/F-081/F-103/F-104 the four CI-only reds, diagnosed together (F-101 is RESOLVED — it was the RB-Rhat 1e-8 pin, fixed in 12; F-103 = gauge harm_pred 0.01071 Linux-only, restore to T1 after diagnosis; F-104 = socket intermittent with the 4-worker-nodes-for-`cores = 2L` clue + retraction trail — see rows + report 12 findings 2-3); F-100 compliance-harness property-name sweep + fixture re-baseline; F-073 guard closures — all four holes, incl. the planted-δ every-run pin design; F-074 SUPERSEDED here — resolves via brief 19's post-fix re-measure (16 cut the discriminator cells after root-causing F-075; see rows); F-114 extract_arguments main_effect_indices gap; F-077 optional; GGM fixture extending the report-10 slab-frame identity pin. F-075 program MOVED OUT to brief 16 per MM's release-gate ruling. PLOT FOLLOW-UPS from 11: F-107 remove the inert `evidence_threshold` arg (NOTE: may be re-purposed by the three-panel redesign — check 11's round-2 outcome first); drop F-105's call-site workarounds (17's function guard is merged). FROM 17: F-109 sparse-coding verification + TRUE NEWS entry if confirmed; F-106 NEWS clause recheck against 11's merged ring; prior-sensitivity.Rmd onto the docs-site BAKE pattern (MM 2026-08-02: real call shown eval=FALSE, baked trimmed `ps` object loads hidden, print/plot LIVE at build — drift-proofing preserved; bake script committed with rebake-at-release rule; 56 s → ~10 s; enable `plot(ps)`/`head(ps$edges)` in the same pass. Lead history check 2026-08-02: NO .rds ever under this repo's vignettes/ (full --all history), no CRAN complaint on record, cran-comments.md doesn't exist yet — MM half-recalled a possible CRAN complaint; the pattern's live home is bgms-docs fixtures/bake.R. RAILS: vignette .rds stays double-digit KB (ps object only, NEVER fit objects — site's are 26–133 MB), report the tarball delta, `--as-cran` gate must stay at the 2 baselines — any new vignette-file NOTE reverts the change); F-018/F-031 23-example uncapped-cores family sweep if MM wants examples covered) | Opus | draft after 12 | — |
| 14 NEWS.md verification + reconstruction for the 0.1.6.3 reader (F-001 blocker fix, F-023 sentence, F-029 line-by-line verification; claims table; NEWS.md-only branch) | Opus | done — **merged to develop** (commit `80a28dfb`). F-001 CLOSED (fixed pre-review by `45a4fae7` #187; agent re-verified line-by-line + added the pooled-everywhere clause); F-023/F-029 closed (7/11 F-029 items had NO entry); 98/98 pre-existing claims verified — 4 corrected (2 outright false at HEAD → F-082..F-085), 8 never-shipped dev entries removed; parse gate via the actual markdown-NEWS reader (33 rows clean). MM answers (2026-08-01): Q1 referent unknown — entry stays removed, no relocation; Q3 keep the nine-bullet hierarchical block this release; Q4 intended (prior classes + default-family change; deprecated formal keeps Cauchy). MM's ratification read DELIVERED 2026-08-01 → F-096 (B6 dev-history reframe — lead tag-verified the released pair was mutually consistent; B7 dropped; uniform never-shipped rule for Bug fixes; three clarifying additions); amendments ride brief 17 task 14. A final skim after 17 merges closes the box | `reports/14-news-verification.md` |
| 15 vignette + man-page accuracy sweep (all vignettes claim-by-claim vs installed build; 23 new exports' man pages; nats convention; F-006 cross-check; ANALYSIS ONLY — fixes batched later) | Opus | done 2026-08-01 — ~120 claims across 5 vignettes + 23 exports; renders clean; 21/23 examples clean, all defaults = formals. TWO MAJORS: F-086 (sensitivity vignette's report block is a never-matching mock), F-087 (diagnostics vignette INVERTS the BF reading — its only worked BF example). Plus F-088..F-095 (log10 site, transition-count naming, phantom report elements, `print.bgms_verdicts` subset error, second NA route, "two orders" runtime messages, half-right ESS sentence, chain-boundary transition fabrication); F-006 independently re-confirmed; F-026/F-031/F-045 re-confirmed; D-12 already resolved by brief 14. All fixes → brief 17 | `reports/15-docs-accuracy-sweep.md` |
| 17 docs + small-defect fix batch (F-086 live chunk, F-087 inverted reading, F-088/F-089/F-090/F-092/F-094 vignette wording, F-091 verdicts-print guard + cosmetics, F-093 runtime messages, F-095 chain-boundary fix, F-002's comparison-vignette sentence, F-026 minimum intro fixes, F-031's two examples, F-049-Rd harmonization verify; F-096 NEWS ratification amendments; F-097/F-099 archive-and-remove the dormant analytic law + self-contained comments) | Opus | **done — merged `ec1c91a4` 2026-08-02** (+ lead addendum F-105/F-106/base-C). All gates green (both tiers 0F/0W; 5 vignettes clean, prior-sensitivity 56 s live chunk; `--as-cran` 2 baseline NOTEs ×2 runs). Lead-verified: both never-shipped drops at the tag (warning string + caches ABSENT, min-shift read directly — RATIFIED), law removal grep-clean, archive branch content + PUSHED, paths-ignore ×4 (PR triggers untouched), F-095 per-chain scan + the two falsified-reference tests, F-087 inline-R. Spawned F-109 (sparse-coding TRUE-fix candidate → 13); F-106 recheck at 11's integration; F-031/F-018 examples-family note. | `reports/17-docs-fix-batch.md` |
| 16 F-075 resolution: replicate (10 data seeds) → localize (scale sweep, selection, adaptation) → fix proposal. **RELEASE GATE per MM 2026-08-01**; the authorized heavy lane (sequenced fits); analysis-only, fix lands as a separately reviewed batch | Opus | **issued 2026-08-01** — `briefs/16-f075-resolution.md`. Phase 0 done (mixing refuted; localized to quasi-separated avoidact edges; two hypotheses). **Phase A done 2026-08-02** (10/10 seeds): systematic g1 inflation + decomposition into ~4–5% healthy-edge offset (=F-074?) and degenerate-edge runaway; 09's noise-3.8× refuted; detection unharmed; 09's sim design flaw flagged (planted edges manufacture the degeneracy). Phase B interim 2026-08-02: **profile log-PL fired — DEFECT, not prior** (compare solution fits worse by ~0.08–0.13/obs on every runaway cell, deficit tracks the runaway); sweep curves FLAT 1 → 0.5 (prior lever dead twice over); agent withdrew the induced-prior story for F-074's offset (4.3% on indicator-0 edges) → contamination hypothesis, null-split + dose–response cells discriminate. **DONE 2026-08-02 — ROOT CAUSE FOUND**: `collapse_categories_across_groups()` intersection semantics (report 16, committed `df10d78a` with 3 figures). Lead-verified independently (code read + fresh reproducer run + Boredom negative check). Compensation hypothesis refuted (mains free at indicator 1); profile-deficit reading corrected by the agent (damage measure, not mistargeting); all knobs inert because the defect is upstream. Spawned F-110–F-114; F-074 left open (cells cut, correct call); fix = brief 19. | `reports/16-f075-resolution.md` |
| 11 package-wide plot restyle onto the brief-10 style module (F-067; every remaining plot, incl. the sensitivity plot's big title and F-070 clipping; before/after renders per figure for MM's judgment). GREW from MM's report-10 verdict (2026-08-01): (a) edge panel drops the verdict-word annotation in every case; (b) "PIP" → self-explanatory inclusion label (lead proposal "P(included)"); (c) three decimals via the style constant; (d) **`plot.bgmCompare` must show difference Bayes factors with the same evidence conventions `plot.bgms` uses** (MM requirement, from the F-079 exchange); (e) compare node rings qgraph-pie → `probability_wheel()` (report-10 deviation 8) | Opus | **executed 2026-08-02** — branch `fix/plot-sweep`, 8 commits, local (no push per brief). Gate green (8253 pass / 0 F / 0 W; snapshots: `_snaps/plot-methods.md` only). LEAD CODE-VERIFIED: forbidden paths untouched (independent diff check); parity pin real (`test-plot-methods.R:108-133`, both units' evidence bands asserted identical); single renderer `draw_verdict_network()`; legend A/B behind `getOption("bgms.network_legend", "verdict")` (review affordance — promote-and-delete in 13 per MM's choice); 21 render pairs + INDEX + make_renders.R committed. Base note: repo's LOCAL develop ref 23 commits stale — agent correctly based on origin/develop (recurring footgun, conventions updated). Spawned F-105/F-106 (→17 addendum), F-107 (remove, →13), F-108 (MM ratify, →13). RENDER VERDICT ROUND 1 (2026-08-02): substantial revision — edge-panel trio LOCKED; legend B + raw-BF thresholds RATIFIED; evidence-width encoding required; band/captions/jargon cut; wheel-beside-node REVERSED to qgraph rings; sensitivity trajectories connected; compare-groups K>2 redesign; group_tag vacuous-parenthetical fix. Round 2 EXECUTED 2026-08-02 (11 commits total): three-panel edge evidence plot per the corrective addendum; locked trio BLOB-VERIFIED byte-identical; legend dissolved (option deleted, raw-BF rule lines); all cuts and reversals applied; group_tag fixed; groups paged; trajectories connected; selection-off weights panels; empty-network stop() removed. Two agent judgment calls pending MM (threshold rule line; groups page without difference panel — lead endorses both). K>2 RULING (lead): three panels for any K, supported panel UNWEIGHTED when K>2 (magnitudes from type="groups"/extract_group_params) — relay addendum issued, K=3 render pair to follow. ROUND-2 VERDICT IN (MM, six items — sign legends out everywhere, keys name evidence classes with BF rules, bigger panel titles, node-ring line out, edge-panel captions out on all four incl. the trio BY MM ORDER, no-selection overlap resolved by legend removal): round-3 relay issued with the K=3 pair. MERGE gated on MM round-3 look (deltas only). | `reports/11-plot-sweep.md`; revision round in flight |
