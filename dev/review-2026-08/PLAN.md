...
| 06 bgmCompare defect + units batch (F-061 sensitivity misalignment FIRST, F-056/F-057 input handling, F-060 verdicts print, F-021 method (adapt the parked patch), F-063 centrality removal, F-062a–d plots, nats-everywhere conversion, F-065 startup silence) | Opus | **issued 2026-08-01** — repurposed from "cross-path consistency", which moves to brief 09 | pending |
...
| 09 bgmCompare cross-path consistency validation (original 06 scope) | Opus | after 06 merges — numbers-level validation belongs after the defect batch, not before | — |
| 10 edge-posterior panel redesign to the JASP/compendium standard (F-066; PIP wheel, prior overlay, conditional density, no Savage-Dickey dots) | Opus | drafted 2026-08-01 — relay after 06 merges (same-file conflict on `R/plot_bgms.R`); MM reviews before/after renders pre-merge | — |
