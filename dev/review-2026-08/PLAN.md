...
| 10 edge-posterior panel redesign to the JASP/compendium standard (F-066; PIP wheel, prior overlay, conditional density; Savage-Dickey dots exactly when `edge_selection = FALSE`; extracts the `R/plot_style.R` house-style module) | Opus | drafted 2026-08-01, expanded per MM — relay after 06 merges (same-file conflict on `R/plot_bgms.R`); MM reviews before/after renders pre-merge | — |
