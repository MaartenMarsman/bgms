...
* `RcppParallel` is constrained to `< 6.0.0`. Version 6.0.0 replaced the TBB backend with oneTBB 2022.0.0, under which multi-chain sampling on Windows no longer reproduces the single-chain (serial) draws bit-for-bit. The constraint keeps the reproducible 5.x backend while the interaction is investigated.
