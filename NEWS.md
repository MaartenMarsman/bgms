...
* `extract_rb_inclusion_probabilities()`: Rao-Blackwellized posterior inclusion probabilities. For each indicator update the sampler now records the one-step draw `J = gamma + (1 - 2 gamma) alpha` (pre-move state `gamma`, birth/death acceptance probability `alpha`), stored per chain and per edge in `fit$raw_samples$rb_inclusion` alongside the raw indicator draws. Averaging `J` is a lower-variance estimator of the inclusion probability that stays interior: edges whose raw average saturates at 0 or 1 purely because the chain never flipped in a finite run receive an interior, finite-Bayes-factor estimate. The RB draw is continuous, so the standard MCSE/ESS/Rhat machinery applies to inclusion inference. Available for `bgm()` (ordinal, continuous/GGM, and mixed) and `bgmCompare()` (difference indicators); the raw indicator draws and transition counts are unchanged and remain the default reported inclusion probability. The estimator inherits the chain's mixing and does not rescue a chain that failed to explore the model space; for edges with overwhelming per-iteration evidence the acceptance probability can underflow to zero in double precision, so those still report a machine 0/1.
