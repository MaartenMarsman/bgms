...
* The hierarchical prior's absolute-moment surface is now validated at three `gamma_prior()` diagonal shapes -- 0.5, 1, and 2 -- and deploys on the range they span, where before it deployed only at the exponential shape 1; the interior of that range is interpolated, not measured, and a shape outside it keeps the additive kernel. Scored against block-Gibbs gold at shapes 0.5 and 2, `eta` 1 and 2, both component families, over two anchor-build seeds: the surface holds a median of 0.0001 to 0.0013 nats and at most 0.0027 across the eight cells, against 0.016 to 0.40 for the additive kernel it replaces there, and the boundary-slope tail was spot-checked at a block of 100 variables in every cell (0.0001 to 0.0059). The upper end of the range is set by the anchor oracle rather than the surface: at a non-unit shape the oracle's row update is an independence-Metropolis step whose acceptance falls away from shape 1 (measured at 20 nodes and density 0.9: 80% at shape 2, 70% at 2.5, 60% at 3, 35% at 4, and 1 to 12% at shape 5), and at shape 5 the anchor Monte-Carlo error runs 50 to 500 times the shape-1 anchor error and is not restored by 94 times the sweeps. Anchors at a non-unit shape are therefore run at a raised sweep budget, resolved by matching the measured anchor spread to the shape-1 reference. The fence message now names the validated shapes and this reason. The generalized Gauss-Laguerre quadrature carrying the shape through the constants was certified against brute-force integration first, including at the shape below 1 where the diagonal prior is singular at the origin, so a surface result is attributable to the surface.

* Fixed: the surface's block-Gibbs anchor helpers did not pass the cell's Gamma shape to the oracle, so anchors were drawn at the exponential shape while the constants carried the cell's own. The alpha != 1 fence kept this dormant, and it is corrected as part of opening that fence.

* Fixed: with a non-unit `gamma_prior()` diagonal shape, the hierarchical prior's absolute-moment surface was built, cached and attached, and then ignored. The sampler's per-edge normalizer kept its own shape test from the release in which the surface served the exponential shape only, so when the validated range widened the two layers disagreed and every non-unit-shape fit was served the coarser additive kernel while the surface it had just paid to build went unused. Measured on a dense common-neighbour block of 30 variables at shape 2, the served value was 0.069 nats from the surface value, more than twenty times the accuracy the surface is stated to hold. The deployment range now has a single owner: the fit decides which cells get a surface, and the sampler uses whatever it is given. A regression test asserts that the value a fit actually receives is the surface value at each of several shapes, rather than only that it differs from the additive one.

* The absolute-moment surface's `gamma_prior()` diagonal shape range now runs to 10, from 2. It is scored against a block-Gibbs reference at shapes 0.5, 1, 2, 3 and 5 -- every interior cell inside the 0.003-nat envelope, against 0.016 to 0.40 nats for the additive kernel it replaces there -- and the interior of the range is interpolated, not measured. At shape 10 the guarantee is of a different kind and is stated separately: the diagonal has concentrated far enough that the entire mediated correction is bounded by 0.00028 nats across the scored blocks at a standardized rate of 2 or below, so the correction is negligible rather than merely well approximated. That bound is set at the upper end of the scored rate range, since the correction grows with the rate, and it does not extend above it. The upper end of the old range was set by the anchor simulator rather than the surface, and the simulator was replaced (see below), so the range moved for that reason and not because the surface changed.

* The block-Gibbs anchor simulator behind the surface now samples the precision diagonal by slice sampling instead of an independence-Metropolis step. At a non-unit diagonal shape the old proposal came from the exponential-shape conditional and was corrected by a ratio that grows with the diagonal entry, so its acceptance collapsed as the shape rose (about 1% at shape 5) and the sampler froze while still reporting moments -- a wrong answer rather than a noisy one. The replacement has no acceptance step in that coordinate and its efficiency per sweep is flat in both shape and block size. The exponential-shape path is unchanged and bit-identical, seed for seed. The one-time cached surface build stays inside its budget: 24.5 seconds at the exponential shape and 43 to 47 seconds at a non-unit one, single-core, at the full 80-variable range.

* Fixed: the fixed quadrature grid carrying the interaction slab through the hierarchical prior's pair integrals lost accuracy at a large `gamma_prior()` diagonal shape. A larger shape concentrates the integrand into a narrow spike, and the fixed rule stopped resolving it: measured against a converged rule at a standardized rate of 1, the error in the tabulated constants ran 2.4e-06 at shape 10, 2.4e-04 at 15 and 3.9e-03 at 20, the last of which exceeds the accuracy the surface itself is stated to hold. The rule is doubled, which restores agreement to 1e-07 or better at every shape and rate tested through 20, at a cost of about 0.4 seconds on a once-per-fit table build. The constants move by 6e-11 in the resulting per-edge log normalizer ratio, far below every validated envelope, so no recorded result changes.

* Fixed: the hierarchical prior's tabulated pair integrals ended at a smaller argument than the saddle-point integration that consumes them. The table is interpolated, and a read past its end clamps to the edge value, so the last 22 units of that integration carried a constant plateau where the integrand should have decayed. The table now covers the range that is read, and the two grid domains assert their compatibility where they meet, because a clamp of this kind is invisible at the call site. The range is extended by appending nodes rather than re-spreading the same number over a wider interval, so every node the shorter table carried is unchanged to the bit. On the cells the package deploys at the exponential diagonal shape the correction moves the tabulated saddle transform by at most 1.8e-6 relative and the per-edge log normalizer ratio by at most 4.3e-7 nats, three to four orders below the 0.003-nat envelope the surface accuracy claims are stated in, so no recorded validation result changes and the effect on a fit is not observable. The plateau is present in released `bgms`. It matters only where the integrand decays slowly enough to still carry mass at the old edge, which is why it surfaced while certifying the constants at diagonal shapes above the previously tuned range.
