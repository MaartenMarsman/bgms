...
* `posterior_predictive_check()` draws one replicated dataset per posterior draw, so parameter and structure uncertainty propagate into the replicates, and compares three statistics with their posterior predictive intervals: the category proportions of each variable, the Spearman correlation of each variable pair, and the sum-score distribution, which depends on the joint dependence among all variables and can therefore fail while the other two pass. `plot()` uses two mark types: an interval scatter of observed against predicted for the element-wise statistics, which carries both coverage and systematic departure in one panel, and a predictive band for the sum-score distribution over its ordered support. Each panel states the number of misses a calibrated model is expected to produce, so the expected handful does not read as a failure.
...
* The predictive checks cover discrete variables in this release; a fit containing continuous variables errors rather than reporting a statistic with no analogue there.

* New vignette "Checking your fitted model": `verdicts()` and the verdict-encoded network, then the two predictive checks, with the two-layer reading (coverage against systematic pattern) worked through on Wenchuan, where the conditional predictions are usable while the joint dependence is understated.
