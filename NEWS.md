...
* New vignette "Checking your fitted model": `verdicts()` and the verdict-encoded network, then `calibration_check()`, then a worked example of building a predictive display directly on `simulate()` — the joint-level check is an analyst's reading of a display, not a pass/fail statistic, so the vignette shows how to roll one rather than calling a blessed function.
