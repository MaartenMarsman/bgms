...
* `prior_sensitivity_check()` is rebuilt on a warm-started refit grid. It refits the model at a grid of interaction slab scales (multipliers of the fit's chosen scale, default `0.5 / 1 / 2.5`) and classifies every edge at every scale, with a Monte Carlo wobble guard calibrated from a replicate refit at the chosen scale: an edge is called scale-sensitive only if its verdict changes across the grid *and* its inclusion-Bayes-factor swing exceeds that wobble, so boundary edges that merely flip between reruns of the same prior are reported as `indistinguishable-from-wobble` rather than as moves. Each refit passes a two-level gate — a refit-level convergence gate (median continuous split-R-hat, bulk Rao-Blackwellized inclusion R-hat, and E-BFMI for NUTS) and per-edge sufficiency (per-chain verdict agreement plus a between-chain-inflated Bayes-factor band) — and the printed report opens with the scale the data prefer (no refit needed). The check works on any `bgm()` fit with edge selection, not only fits with a scale hyperprior. On Wenchuan the whole three-scale check costs about one original fit with warm-started NUTS refits (a new `refit_sampler` argument, default `"same-as-fit"`, carries the adapted NUTS step size and mass matrix). The single-fit conditional-density reweighting curve of the previous version has been removed: on real network data its scale posterior collapses and its importance effective sample size is about 1 across the advertised band (see `dev/audit/2026-07-28-phase-b-review.md` and the git history).
