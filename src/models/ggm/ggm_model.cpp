...
void GGMModel::enable_random_interaction_scale(
...
double GGMModel::get_interaction_scale() const {
...
void GGMModel::update_interaction_scale(int iteration, const WarmupSchedule& schedule) {
    if (!interaction_scale_random_) return;

    // Slab log-density of the included off-diagonal interactions, on the
    // association scale Kyy_ij = -0.5 * Omega_ij (the value the slab prior and
    // the gradient score). Only the slab scale varies; the diagonal Gamma prior
    // stays anchored at s0.
    auto included_slab_logp = [this]() {
        double total = 0.0;
        for (size_t i = 0; i + 1 < p_; ++i) {
            for (size_t j = i + 1; j < p_; ++j) {
                if (edge_indicators_(i, j) == 1) {
                    total += interaction_prior_->logp(-0.5 * precision_matrix_(i, j));
                }
            }
        }
        return total;
    ...
    if (MY_LOG(runif(rng_)) < ln_alpha) {
        // Accept: the slab scale changed, so the cached gradient is stale.
        invalidate_gradient_cache();
    } else {
