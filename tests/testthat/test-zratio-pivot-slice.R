...
test_that("the exponential shape is bit-identical to the pre-split kernel", {
  ...
  # produced by the kernel as it stood before the pivot split, same seed. What
  # they detect is a changed draw stream, which moves them in the leading
  # digits, not the last.
  #
  # The tolerance sits above the cross-platform floating-point floor, not at
  # the precision of the literals. The RNG stream is portable, but S1 averages
  # 2000 sweeps of Cholesky and solve results and logR pushes that through the
  # saddle quadratures, so the last bits follow the platform's BLAS/LAPACK and
  # libm. Measured on CI against these literals: S1 differs by 6.7e-15 relative
  # on macOS (Accelerate), logR by 3.5e-15 on Linux. A 1e-15 pin therefore fails
  # everywhere but the machine that recorded it, while catching nothing a 1e-12
  # pin misses.
  #
  # One consequence: logR also tracks the constants vintage (the theta
  # quadrature going from 64 to 128 points moved it by 6e-15), and that signal
  # is the same size as the platform noise, so this test cannot carry it.
  # Vintage detection belongs to the fixture's static-move guard and the
  # certification ladder, both of which compare runs on one machine.
  r = oracle(dense_block(8), 1, 2000L, 500L, 11L)
  expect_equal(r$S1, 0.1798251775467119, tolerance = 1e-12)
  expect_equal(r$logR, 0.60844358005727470, tolerance = 1e-12)
