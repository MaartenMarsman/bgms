...
nct_check_model = function(x,
  ...
  if(difference_scale <= 0 || is.na(difference_scale) || is.infinite(difference_scale))
    stop("The scale of the Cauchy prior for the edge differences needs to be positive.")
...
nct_reformat_data = function(x, na.action, variable_bool, reference_category, group_indicator) {

  if(length(group_indicator) > nrow(x))
    stop('The group_indicator vector has more elements than x has rows.')
  if(length(group_indicator) < nrow(x))
    stop('The group_indicator vector has less elements than x has rows.')

  if(anyNA(group_indicator)) {
    x = x[!is.na(group_indicator), ]
    group_indicator = group_indicator[!is.na(group_indicator)]
  }

  u = unique(group_indicator)
  if(length(u) != 2)
    stop("The Bayesian Network Comparison Test currently only works for comparing two groups.")

  group_indicator[group_indicator == u[1]] = 0
  group_indicator[group_indicator == u[2]] = 1
  ...
  if(na.action == "listwise") {
    ...
    if(sum(missing_values) == 1)
      warning(paste0("There was one row with missing observations in the input matrix x.\n",
              ...
              call. = FALSE)
    x = x[!missing_values, ]
    group_indicator = group_indicator[!missing_values]
    ...
    if(nrow(x) < 2 || is.null(nrow(x)))
      stop(paste0("After removing missing observations from the input matrix x,\n",
                  "there were less than two rows left in x."))
    if(length(unique(group_indicator)) < 2)
      stop(paste0("After removing missing observations from the input matrix x,\n",
                  "there were less than two unique values left in group_indicator."))
