...
reformat_data_bgm = function(x) {
      ...
      if(length(mis) > 0) {
        for(i in 1:length(mis)) {
          ...
          x[mis[i], node] = min(x[, node], #start value for imputation
                                na.rm = TRUE)
          #non-zero if no zeroes are observed (we then collapse over zero below)
        }
      }
    }
  } else {
    missing_index = matrix(0, nrow = 1, ncol = 1)
    there_is_missing_data = FALSE
  }
  ...
  return(list(x = x,
              ...
              there_is_missing_data = there_is_missing_data))
}
